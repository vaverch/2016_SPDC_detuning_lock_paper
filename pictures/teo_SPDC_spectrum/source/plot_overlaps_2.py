...
delta_p = 0
#Delta_idler_list = [0,3,5,10]
Delta_idler_list = [0,1,2]
#Delta_idler_list = [0]

for Delta_idler in Delta_idler_list:
#Delta_idler = 3
  fig1 = plt.figure(1,figsize=(5.3/2.55, 4.8/2.55))
  ...
  print 'Delta_idler', Delta_idler, 'integral', integral
  lor_idler_signal_norm = lor_idler_signal / integral
  
  ax1_handle = ax1.plot(Delta,lor_signal,'-r', lw=0.8,label = 'signal $\\textrm{F}_\\textrm{s} (\\nu) $')
  ax2_handle = ax1.plot(Delta,lor_idler,'-b', lw=0.8,label = 'idler $\\textrm{F}_\\textrm{i} (\\nu_\\textrm{p} - \\nu) $')
  ax3_handle = ax1.plot(Delta,lor_idler_signal,':k', lw=1.5,label = 'product')
  ax4_handle = ax1.plot(Delta,lor_idler_signal_norm,'-k', lw=1.5,label = 'norml. \nproduct')
  
  
  if g_use_tex == True:
#    ax1.set_xlabel('\\textrm{Detuning} $(\\nu - \\nu (\\ell_\\textrm{s},\\textrm{q}_\\textrm{s},\\textrm{p}_\\textrm{s}))$',**label_font)
    ax1.set_xlabel('$\\textrm{Signal detuning } \\delta_\\textrm{s}$',**label_font)
    if Delta_idler == 0:
